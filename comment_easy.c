...
#define RAD2DEG 57.295779513082320876798154814105   //radian to degree convert coeffi.
#define WHOAMI 0x75 // used to verify the identity of the device. first 6 bit i2c adress. lsgb is AD0 value. def: 0x68
#define PWR_MGMT 0x6B   //configure the power mode and clock source.
#define SMPLRT_DIV 0x19 //sample rate divider of gyroscope outputs
#define ACCEL_CONFIG 0x1C //Accelerometer Configuration register
#define ACCEL_XOUT_H 0x3B 
#define TEMP_OUT_H 0x41 //temp out register
#define GYRO_CONFIG 0x1B //gyro configration register
...
Filter_t FilterX = {            //axis filterin after dot point
    .Q_ANGLE = 0.001f,
    .Q_BIAS = 0.003f,
    .R_MEASURE = 0.03f};
...
uint8_t MPU_Init(I2C_HandleTypeDef *I2Cx)   //reads the configuration values of internal sensors
{
    uint8_t check;
    uint8_t Data;
...
void Read_Accel(I2C_HandleTypeDef *I2Cx, MPU6050_t *DataStruct)  //reads acceleration data from registers
{
    uint8_t Rec_Data[6];
...
void Read_Gyro(I2C_HandleTypeDef *I2Cx, MPU6050_t *DataStruct) //reads gyro data from registers
{
    uint8_t Rec_Data[6];
...
void Read_Temp(I2C_HandleTypeDef *I2Cx, MPU6050_t *DataStruct) //reads temp data from registers
{
    uint8_t Rec_Data[2];
    int16_t temp;
...
void Read_All(I2C_HandleTypeDef *I2Cx, MPU6050_t *DataStruct)  //Reads and filters gyro and acceleration data. It does both in one place with data struct
{
    uint8_t Rec_Data[14];
    int16_t temp;
